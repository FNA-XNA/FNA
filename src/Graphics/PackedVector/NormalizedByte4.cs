...
namespace Microsoft.Xna.Framework.Graphics.PackedVector
{
	public struct NormalizedByte4 : IPackedVector<uint>, IEquatable<NormalizedByte4>
	{
		...
		{
			return (uint) (
				(uint) Math.Round(MathHelper.Clamp(x, -127.0f, 127.0f)) |
				(((uint) Math.Round(MathHelper.Clamp(y, -127.0f, 127.0f))) << 8) |
				(((uint) Math.Round(MathHelper.Clamp(y, -127.0f, 127.0f))) << 16) |
				(((uint) Math.Round(MathHelper.Clamp(y, -127.0f, 127.0f))) << 24)
			);
		}
